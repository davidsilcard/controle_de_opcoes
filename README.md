...
- a seção `Confirmações com evidência pendente` mostra somente os campos que ainda faltam. É possível salvar uma evidência parcial com sua fonte — por exemplo, somente o vencimento oficial — sem inventar strike ou capital; os campos e a fonte já confirmados são preservados.
- quando uma busca documental foi concluída e um campo continua sem prova, use `Concluir auditoria`: a posição sai da fila acionável e vai para `Auditoria concluída — campos sem comprovação`. Ela continua excluída do retorno percentual e da cobertura até aparecer uma fonte nova; `Reabrir auditoria` a devolve à fila de evidências.

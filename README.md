...
- em cada resultado, a tela informa sua procedência: `Cálculo conhecido` quando vem do razão financeiro, `Declaração manual` quando a garantia foi declarada, `Pendência documental` quando falta contrato e `Auditoria concluída sem prova` quando a busca já foi encerrada. Esses estados podem coexistir e não são prova de duplicidade.
...
- `Pendências de cadastro por posição` mostra cada posição somente uma vez e identifica, separadamente, contrato, garantia e vínculo de estoque. Assim, duas pendências na mesma posição não sugerem dois registros. Cada confirmação de contrato exige a fonte usada naquele envio e pode ser parcial; os valores já confirmados não são regravados.
- confirmações de contrato, conclusão documental e reabertura bloqueiam a posição dentro da transação e usam uma gravação restrita aos metadados de desempenho. Esse fluxo não executa a atualização genérica, não sincroniza ledger e não altera estoque.
- A declaração de garantia não exige nota: na `Cash-Covered Put`, a aplicação deriva automaticamente `strike × quantidade`; na `Covered Call`, o valor histórico é preservado no cadastro ou declarado pelo usuário como `garantia_declarada_usuario`.
...
- `Opções.net` pode ser registrado como fonte de strike/vencimento. Quando houver ajuste do contrato durante sua vida, preserve a data e descreva se o valor é original ou final; um strike final atual não deve substituir silenciosamente o strike válido na abertura.
...
- a edicao manual em `Posicoes` agora respeita um contrato de identidade por estrategia: operacoes de `cash_put`, `covered_call`, `ranking` e `estoque` nao podem trocar ticker, ativo, lado, quantidade, preco de entrada, data, modo real/simulado, vinculo legado ou estrategia pela tabela generica. Esse tipo de correcao deve passar pelo fluxo da estrategia ou por ajuste supervisionado.
- exercicio de `Cash-Covered Put` ou `Covered Call` nao pode mais ser encerrado pela tabela generica de `Posicoes`; use o botao da propria estrategia para que caixa, ledger e estoque sejam gravados juntos.
- o estoque usado como garantia de `Covered Call` deve ser mantido como saldo consolidado em `Posicoes`/`Covered Call`: informe somente ticker, quantidade total atual, PM informado, modo real/simulado, data de conferencia e observacao. Esse fluxo atualiza `equity_holdings`, registra evento `MANUAL_SET` e nao cria posicao nem ledger financeiro.
...
- `Covered Call` agora tem guarda isolada de estrategia: bloqueia ativo-base incoerente com o ticker da CALL, data invalida, duplicidade provavel, vencimento/exercicio sem data confirmada e fechamento incoerente.
...
- reparos documentais de PUT exercida usam `uv run python -m opcoes.cli repair cash-put-exercise --put-position-id <id> --exercise-date YYYY-MM-DD --original-strike <valor> --exercise-strike <valor> --purchase-fees <valor> --source-ref "<notas>"`. Sem `--apply`, o comando apenas simula e valida posição, ASSIGN, REALIZED, evento e a cadeia de estoque. Ao aplicar, ele corrige somente a posição auditada: data do exercício, débito líquido, custos do estoque, strike original e strike aplicado. Declarações posteriores de estoque preservam seu saldo e PM final; só as referências históricas "antes" são reatualizadas.
- ajustes por provento têm campos próprios para o strike da abertura, o strike ajustado e a data do ajuste. Use `uv run python -m opcoes.cli repair contract-adjustment --position-id <id> --original-strike <valor> --adjusted-strike <valor> --adjustment-date YYYY-MM-DD --source-ref "<fontes>"` primeiro sem `--apply`. Esse reparo não muda prêmio, resultado, DARF, garantia, vínculo de estoque ou taxas compartilhadas; ele apenas preserva a cronologia documental do contrato.
- em `Desempenho`, um contrato com ajuste auditado mostra o strike ajustado e sua data ao lado da fonte, sem alterar o strike original usado como base histórica da estratégia.
...
- no cadastro de posição, marque `A nota tem despesas compartilhadas sem rateio por operação` e informe a referência exata da nota. Nesse caso, deixe `Taxas` vazias ou em zero: a aplicação não estima custo individual e identifica o resultado como anterior ao custo compartilhado. A marcação não lança a despesa no caixa; confira o total da nota e registre-o uma única vez por fluxo auditável antes de considerar o saldo conciliado.
- em `Cash-Covered Put` > `Movimentar Caixa`, o tipo `Despesa compartilhada de nota` recebe o valor **positivo** total e a referência obrigatória; grava um débito único `SHARED_NOTE_FEE`. Antes de salvar, confira se a referência ainda não existe nas movimentações, pois um segundo formulário novo representa uma nova despesa.
